 ...
 * What's Here:
 * 
 * renderNav(\ProcessWire\PageArray $items, [$options]) - Render a generic navigation list
 * renderSubNav(\ProcessWire\Page $page) - Render Foundation 'side-nav' list for given $page
 * renderBodyNav(\ProcessWire\PageArray $items) - Render Foundation 'side-nav' for bodycopy placement
 * renderBreadcrumbs(\ProcessWire\PageArray $items) - Render Foundation 'breadcrumbs' list
 * renderPagination(\ProcessWire\PageArray $items) - Render pagination links using Foundation classes
 * renderTopBar(\ProcessWire\PageArray $items, [array $options]) - Render Foundation 'top-bar' navigation
 ...
 *
 */
function renderNav(\ProcessWire\PageArray $items, array $options = array()) {
 ...
 *
 */
function renderSubNav(\ProcessWire\Page $page) {

	if(!$page->numChildren(true)) return '';
	if ($page->template == "cases" or $page->template == 'hearings') return '';
 ...
 *
 */
function renderBodyNav(\ProcessWire\PageArray $items, array $options = array()) {
	$defaults = array(
 ...
 *
 */
function renderBreadcrumbs(\ProcessWire\PageArray $items) {
 ...
 *
 */
function renderPagination(\ProcessWire\PageArray $items) {
	...
	$pager = \ProcessWire\wire('modules')->get('MarkupPagerNav');
	$pager->setBaseUrl(\ProcessWire\wire('page')->url);
 ...
 *
 */
function renderTopNav(\ProcessWire\PageArray $items, array $options = array(), $level = 0) {
	...
	foreach($items as $item) {
		...
		$class = '';
		if($numChildren and $item->template != "cases" and $item->template != "hearings") $class .= "has-dropdown ";
		...
		if($numChildren) {
			if ($item->template != "cases" and $item->template != "hearings") {
